...
use ratatui::{
    layout::{Alignment, Constraint, Layout, Margin, Rect, Direction},
    style::{palette::tailwind, Color, Modifier, Style, Stylize},
    text::{Line, Span, Text},
    widgets::{
        block::{Position, Title},
        Block, BorderType, Borders, HighlightSpacing, Paragraph, Row, Scrollbar,
        ScrollbarOrientation, ScrollbarState, Table, TableState, Wrap, List, ListItem,
    },
    Frame,
};
...
impl Tui {
  ...
  pub  fn render_keybindings(&self, f: &mut Frame) {
        let keybindings =  vec![
            ("q".to_string(), "Quit the application".to_string()),
            ("h".to_string(), "Move left".to_string()),
            ("l".to_string(), "Move right".to_string()),
            ("j".to_string(), "Move down".to_string()),
            ("k".to_string(), "Move up".to_string()),
            ("Enter".to_string(), "Select the item".to_string()),
        ];
        ...
        let items: Vec<ListItem> = keybindings.iter()
            .map(|(key, desc)| ListItem::new(format!("{}: {}", key, desc)))
            .collect();

        let list = List::new(items).block(Block::default().borders(Borders::ALL).title("Bindings"));
